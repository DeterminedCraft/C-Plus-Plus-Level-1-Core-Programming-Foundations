...
/*
 ...
 In a program, if you notice that Integral Value(s) (either an integer or a character) will be used to represent something
 instead of using them arithmetically

 then, BEGIN YOUR CODE by creating an enum user-defined data type where this
 process assigns each integral value, a unique label that it represents so that, from now and on,
 only labels will be used to reference their respective integral values that are hard to remember. 
  
 Relying on our memory to remember what the integral value (integer or character) was; let alone what it represents
 ...
 *Note Alpha: Now, these labels (assigned to integers or characters during the creation of the enum user-defined data type),
  are frequently used in conditional statements where they're compared against numerical or character value(s) that are of 
  the same enum user-defined data type ! regardless of where they come from (whether by via user input or receiving it from somewhere 
  in the system).
 ...
 Sometimes in a program, an integer value is used to represent an idea or a state or a concept,
...
/* Have a look at this line of code that creates an enum user-defined data type:
 ...
 Line2:   enDirection direction;
          direction = enDirection::South;    // enDirection::South   stores 226 of the enum user-defined data type that's 'enDirection' 
		                                    //  and then, this 226 is stored in the 'direction' variable that's of the enum user-defined data type that's 'enDirection.

		                                   // In any case - REMEMBER THAT 226's data type is 'enDirection' (Crucial to discern).
 =======================================================================================
- When you declare a variable of type `enDirection` (enum user-defined data type),
  the variable can be assigned an enumeration constant / enumerator (that is `South`) acting as a label,
  ...
  enGender gender;
  gender = enGender::male  // stores 0 (of enGender data type).
...
// Original Notes: Decide what to do with them later even though I made some corrections
 ...
 /* Enumeration (تَعْدَاد) : A user-defined data type.
  ...
  ❌❌ But this is a Wrong Pratice ❌❌
   ...
   Direction = EnDirection::South;  // to the right of the operand stores 112 of data type EnDirection // The variable 'Direction' can only take one of the four defined enum members.
...
int main()
{
	EnColor Color; // Defining a variable 'color' of the data-type 'EnColors'
	// 'color' will have an access to all constant-value members under the user-defined 'EnColors' data type. BUT CAN ONLY STORE one of the defined members.
	EnDirection Direction;
	EnWeek Week;
	EnGender Gender;
	...
	Color = EnColor::Red; // stores 620 (of datas type EnColor) of the label ::Red
...
📌 Note: Enumeration Constants (Enumerators)
 ...
 the integral values (int or char) they are assigned to.
...
------------------------------------------------
 ✅ What an enumeration constant (enumerator) can be assigned to

 Each enumerator can ONLY be assigned an integral value (int or char)
...
------------------------------------------------
 ...
 Enumerator value → integeral (what is actually stored)
...
// Your brain should immediately map:
// → This function RETURNS a value of type EnumType

// Therefore inside the function you EXPECT:
...
// 🔥 CRITICAL RULE:
→ EVERY execution path MUST return one of the enum's enumerators

// Mental interpretation:
→ The function is MAPPING logic → named states(enum values)
